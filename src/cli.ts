...
import { ManualAdapter, Orchestrator, type TaskKind } from './core.js';
...
if (command === 'delegate') {
  const kind = (flag('--kind', 'code') as TaskKind); const objective = flag('--objective'); const file = flag('--context');
  ...
  const result = await new Orchestrator(new ManualAdapter(), { adapter: 'manual', budget: { maxTokens: 4000, maxRetries: 1 }, redact: true }).run(objective, context, kind);
